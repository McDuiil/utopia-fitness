...
const StepsWidget = () => {
  console.log("render StepsWidget");
  const selectedDate = useAppSelector(s => s.selectedDate);
  const steps = useAppSelector(s => s.appData.days[selectedDate]?.steps || 0);
  const goal = useAppSelector(s => s.appData.profile.goalSteps || 10000);
...
export default memo(StepsWidget);
