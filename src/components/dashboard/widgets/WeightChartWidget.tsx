...
const WeightChartWidget = memo(({ appData, t, language }: WeightChartWidgetProps) => {
  ...
  const chartData = useMemo(() => {
    if (!appData?.days) return [{ date: 'N/A', weight: appData?.profile?.weight || 0, bodyFat: appData?.profile?.bodyFat || 0 }];
  ...
  const p = prev.appData?.days ? Object.keys(prev.appData.days) : [];
  const n = next.appData?.days ? Object.keys(next.appData.days) : [];
